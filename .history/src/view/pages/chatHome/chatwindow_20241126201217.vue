    ...
    sendText() {
      if (this.inputMsg) {
        ...
        let data = {
          model: 'gpt-3.5-turbo',
          messages: [
            { role: 'system', content: 'You are a helpful assistant.' }, // 系统消息，通常用于设置系统行为
            { role: 'user', content: chatMsg.msg }  // 用户输入的消息
          ],
          temperature: 1,
          top_p: 1,
          max_tokens: 500,
        ...
        chatgpt(data).then((res) => {
          ...
          // 更新 ChatGPT 消息内容
          this.chatList[this.chatList.length - 1].msg = responseMessage;
        });
